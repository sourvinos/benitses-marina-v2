...
namespace Items {
    ...
    [Collection("Sequence")]
    public class Items04Post : IClassFixture<AppSettingsFixture> {
        ...
        public Items04Post(AppSettingsFixture appsettings) {
        ...
        [Theory]
        [ClassData(typeof(CreateValidItem))]
        public async Task Simple_Users_Can_Not_Create(TestItem record) {
            await Forbidden.Action(_httpClient, _baseUrl, _url, _actionVerb, "simpleuser", Helpers.SimpleUserPassword(), record);
        }

        [Theory]
        [ClassData(typeof(CreateInvalidItem))]
        public async Task Admins_Can_Not_Create_When_Invalid(TestItem record) {
        ...
        [Theory]
        [ClassData(typeof(CreateValidItem))]
        public async Task Admins_Can_Create_When_Valid(TestItem record) {
