...
namespace Items {
    ...
    [Collection("Sequence")]
    public class Items03GetForBrowser : IClassFixture<AppSettingsFixture> {
        ...
        private readonly string _url = "/items/getForBrowser";

        #endregion

        public Items03GetForBrowser(AppSettingsFixture appsettings) {
        ...
        public async Task Active_Users_Can_List(Login login) {
            ...
            var records = JsonSerializer.Deserialize<List<ItemBrowserListVM>>(await actionResponse.Content.ReadAsStringAsync(), new JsonSerializerOptions { PropertyNameCaseInsensitive = true });
