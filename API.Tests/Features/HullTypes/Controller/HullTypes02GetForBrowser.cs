...
namespace HullTypes {
    ...
    public class HullTypes02GetForBrowser : IClassFixture<AppSettingsFixture> {
        ...
        public async Task Active_Users_Can_Get_Active(Login login) {
            ...
            var records = JsonSerializer.Deserialize<List<HullTypeBrowserListVM>>(await actionResponse.Content.ReadAsStringAsync(), new JsonSerializerOptions { PropertyNameCaseInsensitive = true });
