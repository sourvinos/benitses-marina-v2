...
namespace TaxOffices {
    ...
    public class TaxOffices02GetForBrowser : IClassFixture<AppSettingsFixture> {
        ...
        public async Task Active_Users_Can_Get_Active(Login login) {
            ...
            var records = JsonSerializer.Deserialize<List<TaxOfficeBrowserListVM>>(await actionResponse.Content.ReadAsStringAsync(), new JsonSerializerOptions { PropertyNameCaseInsensitive = true });
            Assert.Equal(146, records.Count);
        }
