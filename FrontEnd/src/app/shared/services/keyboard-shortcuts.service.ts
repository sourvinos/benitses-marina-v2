...
export class KeyboardShortcuts {
    ...
    constructor(zone: NgZone) {
        ...
        this.zone = zone
        this.zone.runOutsideAngular(() => { window.addEventListener('keydown', this.handleKeyboardEvent) })
    }
    ...
    private getKeyFromEvent(event: KeyboardEvent): string {
        ...
        if (key.startsWith('U+')) { key = String.fromCharCode(parseInt(key.slice(2), 16)) }
