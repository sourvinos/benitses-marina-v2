...
namespace API.Features.Reservations {
    ...
    public class ReservationsController(IReservationRepository repo, IReservationValidation validation) : ControllerBase {
        ...
        public async Task<ResponseWithBody> PostAsync([FromBody] ReservationWriteDto reservation) {
            ...
            if (await x == 200) {
                var z = repo.Create((Reservation)repo.AttachMetadataToPostDto(ReservationMappingDtoToDomain.DtoToDomain(reservation)));
                return new ResponseWithBody {
                    Code = 200,
        ...
        public async Task<ResponseWithBody> Put([FromBody] ReservationWriteDto reservation) {
            var x = await repo.GetByIdAsync(reservation.ReservationId.ToString());
            if (x != null) {
                var z = validation.IsValidAsync(x, reservation);
                if (await z == 200) {
                    var i = repo.Update((Reservation)repo.AttachMetadataToPostDto(ReservationMappingDtoToDomain.DtoToDomain(reservation)));
                    return new ResponseWithBody {
                        Code = 200,
                        Icon = Icons.Success.ToString(),
                        Body = i,
