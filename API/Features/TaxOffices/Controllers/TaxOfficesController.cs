...
namespace API.Features.TaxOffices {
    ...
    public class TaxOfficesController(ITaxOfficeRepository repo, ITaxOfficeValidation validation) : ControllerBase {
        ...
        public ResponseWithBody Post([FromBody] TaxOfficeWriteDto hullType) {
            var x = validation.IsValid(null, hullType);
            if (x == 200) {
                var z = repo.Create((TaxOffice)repo.AttachMetadataToPutDto(TaxOfficeMappings.DtoToDomail(hullType)));
                return new ResponseWithBody {
                    Code = 200,
        ...
        [Authorize(Roles = "admin")]
        [ServiceFilter(typeof(ModelValidationAttribute))]
        public async Task<ResponseWithBody> Put([FromBody] TaxOfficeWriteDto hullType) {
            var x = await repo.GetByIdAsync(hullType.Id);
            if (x != null) {
                var z = validation.IsValid(x, hullType);
                if (z == 200) {
                    var i = repo.Update((TaxOffice)repo.AttachMetadataToPutDto(TaxOfficeMappings.DtoToDomail(hullType)));
                    return new ResponseWithBody {
                        Code = 200,
                        Icon = Icons.Success.ToString(),
                        Body = i,
